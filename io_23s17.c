/**
 * @file io_piface.h
 * @brief functions for accessing the PiFace add-on for the Raspberry Pi
 * Beispiel vom PiFace-Coding
 */
#include "io_23s17.h"
#include <stdio.h>
#include <stdlib.h>
...
int pfio_init(void)
{
    ...
    spi_write(GPIOB, 0x00); // turn on port B
    spi_write(IODIRA, 0x00); // set port A as an output
    spi_write(IODIRB, 0x00); // set port B as an output
...
void pfio_write_output(uint16_t value)
{
    spi_write(OUTPUT_PORT, value);
    spi_write(OUTPUT_PORTB, value >> 8);
}
